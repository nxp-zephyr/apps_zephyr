...
/* SRAM address to power off */
#define SRAM_PD_ADDRESS		(CONFIG_SRAM_BASE_ADDRESS + 			\
				(CONFIG_SRAM_SIZE * 1024U))
#ifdef CONFIG_SMARTWATCH_REDUCE_RAM
	#if (SRAM_PD_ADDRESS >= 0x30500000)
		#error CONFIG_SMARTWATCH_REDUCE_RAM=y, but need to decrease SRAM size in mimxrt595_evk_cm33.overlay.  Change PDSLEEPCFG2 in &deep-sleep-config, and &sram0 size.
	#endif
#else /* CONFIG_SMARTWATCH_REDUCE_RAM=n */
	#if (SRAM_PD_ADDRESS < 0x30500000) /* if less than all 5MB SRAM */
		#warning CONFIG_SMARTWATCH_REDUCE_RAM=n, but not all of SRAM available.  SRAM size can be increased in mimxrt595_evk_cm33.overlay.  Change PDSLEEPCFG2 in &deep-sleep-config, and &sram0 size.
	#endif
...
#ifdef CONFIG_SMARTWATCH_REDUCE_RAM
	assert(SRAM_PD_ADDRESS == 0x30300000);
